...
const FeaturedGallery = () => {
  ...
  useEffect(() => {
    if (isInView && !isAnimating) {
      ...
      // Calculate total animation duration for sequential bars
      // Ultra-fast timing: 0.3s animation + 0.2s delay between cards
      const numProjects = 6;
      const barDuration = 300; // 0.3s in ms
      const sequentialDelay = 200; // 0.2s gap between each bar start
      const totalDuration = (numProjects - 1) * sequentialDelay + barDuration + 200; // +200ms buffer
